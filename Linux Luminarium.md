...
<font color="#0d8f48">T表示被悬挂起来，由于你的crl+z，S表示进入的睡眠模式，等待你的输入，R表示现在正处于活跃状态，+表示现在在表面运行，而不是在背后运行。</font>
...
第三个参数指的是从上次密码更改以后的天数
## Perceiving Permissions
Subject Object right : 主体，客体，权力：
可以使用 ls -la进行查看
针对directory他的读写权限在于能否用ls查看他的文件夹下面有哪些文件，能否用touch，rm，mv等去修改下面的文件。那用户和用户组有什么区别吗？
![alt text](./mypicture/image.png)
用户组是指像用户这一类的用户都可以有这个权限，例如，mysql也是一个用户，root也是一个用户，zzd-zjz也是一个用户。
当我使用id查看当前我的用户是属于什么subject组的时候，会出现 gid和groups，这二者的区别在于gid是该用户的默认属于的用户组，而groups也是用户组，但不是默认的。
我们可以用chmod [u,g,o]+/-w,r,x 文件名来修改该文件的权限，其中u,g,o是这个文件针对用户，用户群和其他人
用;来表示两个命令同时运行，&&来表示只有当第一个运行成功了，才能运行第二个。
用||来表示第一个运行不成功了，才能运行第二个。
其中有一个知识点涉及到的是.sh,它是很多Linux的命令代码集合起来的文件，用bash进行运行，例：
/challenge/run;/challenge/pwn用bash运行的化，就会分别运行这两个程序。
